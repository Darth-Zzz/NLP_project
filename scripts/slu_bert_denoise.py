...
from utils.denoise_example import Example
from utils.denoise_batch import from_example_list
from utils.vocab import PAD
from model.slu_bert_denoise_model import SLUBertDenoise
...
model = SLUBertDenoise(args).to(device)
...
def set_optimizer(model, args):
    ...
    for n, p in model.named_parameters():
        if "denoise" in n:
            denoise_params.append(p)
        else:
            other_params.append(p)
    grouped_params = [{"params": other_params, "lr": args.lr * 0.1}, {"params": denoise_params, "lr": args.lr}]
    optimizer = Adam(grouped_params)
    return optimizer
...
if not args.testing:
    ...
    nsamples, best_result = len(train_dataset), {"dev_acc": 0.0, "dev_f1": 0.0}
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            }, open('checkpoints/bert_denoise_model.bin', 'wb'))
