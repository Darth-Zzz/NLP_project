...
if args.testing:
    check_point = torch.load(open('checkpoints/denoise_model.bin', "rb"), map_location=device)
...
def predict():
    ...
    for ei, example in enumerate(test_json):
        for ui, utt in enumerate(example):
            ...
            ptr += 1
    json.dump(test_json, open(os.path.join(args.dataroot, "dev_prediction_denoise.json"), "w"), indent=4, ensure_ascii=False)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            }, open('checkpoints/denoise_model.bin', 'wb'))
