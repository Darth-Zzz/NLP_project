...
class SLUTagging(nn.Module):
    ...
    def forward(self, batch):
        ...
        denoise_ids = batch.denoise_ids
        gt_embed = self.word_embed(denoise_ids)
        embed = self.word_embed(input_ids)
        denoise_inputs = rnn_utils.pack_padded_sequence(embed, lengths, batch_first=True, enforce_sorted=False)
        denoise_rnn_out, _ = self.denoise_layer(denoise_inputs)  
        denoise_out, denoise_len = rnn_utils.pad_packed_sequence(denoise_rnn_out, batch_first=True)
        denoise_loss = self.loss(gt_embed, denoise_out)

        packed_inputs = rnn_utils.pack_padded_sequence(denoise_out, lengths, batch_first=True, enforce_sorted=False)
